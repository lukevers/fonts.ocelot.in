	...
	app.get('/css', function(req, res) {
		...
		if (!query['font-family']) {
			...
			if (!files.contains(query['font-family'])) {
				...
				var content = '@font-face {\nfont-family: '+query["font-family"]+';\nsrc: url('+config.url+'/fonts/'+query["font-family"]+'.woff) format("woff"), url('+config.url+'/fonts/'+query["font-family"]+'.ttf) format("ttf"), url('+config.url+'/fonts/'+query["font-family"]+'.otf) format("otf"), url('+config.url+'/fonts/'+query["font-family"]+'.eot) format("eot")\n}';
				res.send(content);
			}
		}
	});
